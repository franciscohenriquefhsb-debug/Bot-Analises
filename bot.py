...
API_KEY = os.getenv("API_KEY") # API-Football pra outras ligas
...
def calc_over_prob(media_gols):
    ...
    return {"o25": over_25 * 100, "u25": under_25 * 100}
...
def get_team_stats(team_id, league_id, season):
    ...
    return {
        "casa_faz": float(data["goals"]["for"]["average"]["home"] or 0),
        "casa_sofre": float(data["goals"]["against"]["average"]["home"] or 0),
        "fora_faz": float(data["goals"]["for"]["average"]["away"] or 0),
        "fora_sofre": float(data["goals"]["against"]["average"]["away"] or 0)
    }
...
async def jogos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if liga_code == "COPA":
            # API grátis worldcup26.ir pra Copa 2026
            url = "https://worldcup26.ir/get/games"
            r = requests.get(url, timeout=15)
            all_games = r.json()

            # Filtra jogos futuros e pega os 5 próximos
            ...
            for jogo in fixtures:
                ...
                # Usa média histórica da Copa: 2.6 gols
                media_esperada = 2.6
            ...
            # API-Football pras outras ligas
            ligas = {
                "BR1": 71, "PL": 39, "SA": 135, "LL": 140, "BL1": 78,
                "L1": 61, "LIBERTA": 13, "SULA": 11, "COPABR": 73
            }

            if liga_code not in ligas:
                await update.message.reply_text("Liga inválida. Use: COPA, BR1, PL, SA, LL, LIBERTA, SULA")
                return
            ...
            for jogo in fixtures[:5]:
                ...
                try:
                    ...
                    msg_final += f"🏆 {home['name']} x {away['name']} - {hora}\n"
