...
async def jogos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not fixtures:
            await update.message.reply_text(f"Nenhum jogo da {liga_code} hoje {hoje}.")
            return

        msg_final = f"📊 ANÁLISES {hoje} - {liga_code}\n\n"
        jogos_encontrados = 0

        for jogo in fixtures[:5]: # Limite da API grátis
            ...
            try:
                ...
                # Média esperada de gols no jogo
                ...
                msg_final += f"O0.5: {probs['o05']:.1f}% | U0.5: {probs['u05']:.1f}%\n"
                msg_final += f"O1.5: {probs['o15']:.1f}% | U1.5: {probs['u15']:.1f}%\n"
                msg_final += f"O2.5: {probs['o25']:.1f}% ✅ | U2.5: {probs['u25']:.1f}%\n"
                msg_final += f"O3.5: {probs['o35']:.1f}% | U3.5: {probs['u35']:.1f}%\n"
                msg_final += f"O4.5: {probs['o45']:.1f}% | U4.5: {probs['u45']:.1f}%\n"
                msg_final += "───────────────\n"
